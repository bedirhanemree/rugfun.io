...
const ctx = canvas.getContext("2d");// Socket.io bağlantısını kur
const socket = io();canvas.width = window.innerWidth;
canvas.height = window.innerHeight;const mapWidth = 15000;
const mapHeight = 15000;let gameStarted = false;
let gameOver = false;let player = {
...
updateRadius(player);// Tüm oyuncuların listesi (sunucudan alınacak)
let players = [];
let target = { x: canvas.width / 2, y: canvas.height / 2 };let boostActive = false;
let boostTimer = 0;let particles = [];const foodTypes = [
...
jeetAngryImage.src = "jeet_angry.png";let jeets = [];
for (let i = 0; i < 100; i++) {
...
socket.on('init-players', (serverPlayers) => {
  ...
  player.name = name;  if (coinImageInput.files.length > 0) {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        gameStarted = true;
        // Oyuncunun ID'sini socket ID ile eşleştir
        player.id = socket.id;
...
if (jeet.shakeTimer > 0) jeet.shakeTimer--;
  ...
  ctx.fillRect(viewX, viewY, viewWidth, viewHeight);  ctx.strokeStyle = "#333";
  ctx.lineWidth = 1;
  ...
  ctx.font = "14px Arial";
  ctx.textAlign = "center";
  ctx.fillText(p.name, p.x, p.y - p.radius - 15);  ctx.font = "12px Arial";
  ...
  ctx.fillText("Press R to Restart", canvas.width / 2, canvas.height / 2 + 50);  window.addEventListener("keydown", restartGame);
